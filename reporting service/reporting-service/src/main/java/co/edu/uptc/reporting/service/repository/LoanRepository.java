...
public interface LoanRepository extends JpaRepository<LoanEntity, Long> {
    ...
    @Query(value = "SELECT HOUR(start_time) AS hour, COUNT(*) AS frequency " +
            "FROM loans GROUP BY HOUR(start_time) ORDER BY frequency DESC", nativeQuery = true)
    List<HourFreqProjection> findHoursOrderByFrequencyDesc();

 
    @Query(value = "SELECT HOUR(start_time) AS hour, COUNT(*) AS frequency " +
            "FROM loans GROUP BY HOUR(start_time) ORDER BY frequency ASC", nativeQuery = true)
    ...
    @Query(value = "SELECT u.academic_program AS program, WEEK(l.loan_date) AS week, COUNT(*) AS count " +
            "FROM loans l JOIN users u ON l.user_code = u.code " +
    ...
    @Query(value = "SELECT classroom_code AS classroom, COUNT(*) AS frequency " +
            "FROM loans GROUP BY classroom_code ORDER BY frequency DESC", nativeQuery = true)
